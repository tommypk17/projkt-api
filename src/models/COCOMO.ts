...
export class Cocomo {
    ...
    getCocomoModelNames = (): string[] => {
        ...
        let cocomoModelList = FileService.readCocomoModelList();
        Object.keys(cocomoModelList).forEach((v: string, i: number, a: string[]) => {
            modelList.push(v);
        });
        return modelList;
    }
    ...
    getCocomoRatingsByName = (ratingName: string) => {
        let cocomoRatingList = FileService.readCocomoRatingList();
        let cocomoRating = cocomoRatingList[ratingName];
        return cocomoRating;
    };
     ...
     * @param rating
     */
    getCocomoScoreByNameAndRating = (ratingName: string, rating: string): number => {
        let cocomoRatingList = FileService.readCocomoRatingList();
        let cocomoRating = cocomoRatingList[ratingName];
        let cocomoScore: number = cocomoRating['ratings'][rating];
        return cocomoScore;
    };
    ...
    getCocomoModelByName = (modelName: string) => {
        let cocomoModelList = FileService.readCocomoModelList();
        let cocomoModel = cocomoModelList[modelName];
        return cocomoModel;
    };
     ...
     * @param modelVariable
     */
    getCocomoModelByNameAndVariable = (modelName: string, modelVariable: string): number => {
        let cocomoModelList = FileService.readCocomoModelList();
        let cocomoModel = cocomoModelList[modelName];
        let cocomoScore: number = cocomoModel[modelVariable];
        return cocomoScore;
    };
    ...
    calculate = (): CocomoResponse => {
        ...
        Object.keys(this).forEach((v: string, i: number, a: string[]) => {
            if(this.isCocomoRatingField(v) && typeof this[v] == "number"){
                let calculatedScore: number = this.calculateScore(this[v], this.linesOfCode, this.a, this.b);
                calculatedScores.push(calculatedScore);
            }
        });
        let adjustedTime: number = this.calculateAdjustedTime(calculatedScores);
        let developmentTime: number = this.calculateDevelopmentTime(adjustedTime, this.c, this.d);
        let staffingTime: number = this.calculateStaffingTime(adjustedTime, developmentTime);
        let totalHours: number = this.calculateHours(developmentTime);
        return new CocomoResponse(adjustedTime, developmentTime, staffingTime, totalHours);
    }
    ...
    private isCocomoRatingField = (fieldName: string): boolean => {
        return (fieldName != 'linesOfCode' && fieldName != 'model' && fieldName != 'a' && fieldName != 'b' && fieldName != 'c' && fieldName != 'd');
    }
     ...
     * @param b
     */
    private calculateScore = (score: number, loc: number, a: number, b: number): number => {
     ...
     * @param d
     */
    private calculateDevelopmentTime = (adjustedTime: number, c: number, d: number): number => {
     ...
     */
    private calculateStaffingTime = (adjustedTime: number, developmentTime: number):number => {
...
export class CocomoResponse {
    constructor(adjustedTime: number, developmentTime: number, staffingTime: number, totalHours: number) {
